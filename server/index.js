...
async function startMonitoring() {
  ...
  isMonitoring = true;
  console.log(
    "🚀 Starting enhanced monitoring system with second-level precision..."
  );
  ...
  try {
    ...
    const websitesToCheck = activeWebsites.filter((website) => {
      ...
      if (!shouldCheck) {
        ...
        if (remainingTime > 5) {
          // Only log if more than 5 seconds remaining
          console.log(
            `⏳ ${website.name} - next check in ${Math.round(
              remainingTime
            )}s (${user.planType}: ${user.maxCheckInterval}s interval)`
          );
        }
      }
    ...
    const promises = websitesToCheck.map((website, index) => {
      return new Promise((resolve) => {
        setTimeout(() => {
          console.log(
            `🌐 Checking ${website.name} (${website.user.planType} plan, ${website.user.maxCheckInterval}s interval)`
          );
    ...
    console.log(
      `✅ Monitoring cycle completed - checked ${websitesToCheck.length} websites\n`
    );
  } catch (error) {
    console.error("Error in monitoring cycle:", error);
  }
